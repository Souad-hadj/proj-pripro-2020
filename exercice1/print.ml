...
let extract_line l =
  let mots = String.split ~on:'\t' l in
   List.hd_exn mots,List.nth_exn mots 2,List.hd_exn (String.split ~on:'_' (List.nth_exn mots 4));;
           

let extract ic =
  let read () =
    try In_channel.input_line ic with End_of_file -> None in
      let rec f acc = match read () with
        | Some s -> f ((extract_line s) :: acc)
